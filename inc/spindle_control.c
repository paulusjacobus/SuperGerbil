...
//#endif
   ...
   * author Paul
   */
//  TIM_TimeBaseInitStruct.TIM_Period = SPINDLE_PWM_MAX_VALUE - 1;
//  TIM_TimeBaseInitStruct.TIM_Period = (1000000 / settings.spindle_pwm_frequency) - 1;
  ...
  TIM_OCInitStruct.TIM_OCPolarity = TIM_OCPolarity_Low; // TIM_OCPolarity_High to invert the PWM signal 13/08/2018
  ...
  gpio_InitStructure.GPIO_Speed =  GPIO_Speed_2MHz;
  gpio_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; // Test this out 13/08/2018
...
void spindle_stop()
{
		...
		//TIM_SetCompare4(TIM4, 0);//Do we want spindle PWM to be zero for mini Gerbil?
	#endif
